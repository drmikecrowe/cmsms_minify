...
var MUB = {
    _uid : 0,
    _minRoot : '/modules/Minify/min/?',
    checkRewrite : function () {
        var testUri = "/modules/Minify/rewriteTest.js";
        function fail() {
            $('#minRewriteFailed')[0].className = 'topNote';
        }
        $.ajax({
            url : '/modules/Minify/min/?f=' + testUri + '&' + (new Date()).getTime(),
            success : function (data) {
                if (data === '1') {
                    MUB._minRoot = '/modules/Minify/min/';
                    $('span.minRoot').html('/modules/Minify/min/');
                } else
                    fail();                
            },
            error : fail
